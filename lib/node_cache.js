(function() {
  ...
  module.exports = NodeCache = (function(superClass) {
    ...
    function NodeCache(options) {
      ...
      this._ttlPercent = bind(this._ttlPercent, this);
      this._shouldPrefetch = bind(this._shouldPrefetch, this);
    ...
    NodeCache.prototype.set = function(key, value, ttl, cb) {
      ...
      delete this.beingPrefetched[key];
      if (!existend) {
    ...
    NodeCache.prototype._check = function(key, data) {
      if (data.t !== 0 && data.t < Date.now()) {
        ...
        return false;
      } else if (data.t !== 0 && this._shouldPrefetch(key) && this._ttlPercent(data.t, data.s) > this.options.stdPrefetch) {
        this.beingPrefetched[key] = true;
        this.emit("prefetching", key, this._unwrap(data));
        return false;
      } else {
        return true;
      }
    ...
    NodeCache.prototype._shouldPrefetch = function(key) {
      return this.options.usePrefetch && !this.beingPrefetched[key];
    };

    NodeCache.prototype._ttlPercent = function(ttl, start) {
